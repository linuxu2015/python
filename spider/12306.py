...
def listtrain():
	for i in range(0,len(jsondata['data']['datas'])):
		...
		yw = realdata['yw_num']
		wz = realdata['wz_num']
		print '车次：%s 始发站:%s 终点站:%s 上车站:%s 下车站:%s 开车时间:%s 到达时间:%s 车辆类型:%s 历时:%s 一等座:%s 二等座:%s 硬座:%s 硬卧:%s 无座:%s' %(station_train_code,start_station_name,end_station_name,from_station_name,to_station_name,start_time,arrive_time,train_class_name,lishi,zy,ze,yz,yw,wz)
while True:
	listtrain()
	time.sleep(5)
